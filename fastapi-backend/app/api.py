...
from .agent_factory import get_or_create_agent
from .boss_agent_creator import create_swarm_plan, parse_swarm_plan, create_and_execute_swarm
...
@app.post("/api/create_swarm")
async def create_swarm(request: JambaSwarmRequest):
    try:
        swarm_plan = create_swarm_plan(request.task)
        agent_specs = parse_swarm_plan(swarm_plan)
        
        swarm_result = create_and_execute_swarm(request.task)
        
        return JambaSwarmResponse(
            task=request.task,
            plan=swarm_plan,
            agents=[AgentSchema(name=spec["name"], system_prompt=spec["system_prompt"], task=spec["task"], description=spec["system_prompt"][:50] + "...", progress=0) for spec in agent_specs],
            response=swarm_result
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

def parse_swarm_plan(plan: str):
    # Implement logic to parse the swarm plan and extract agent names and system prompts
    # This is just a placeholder
    return ["Agent1", "Agent2"], ["Prompt1", "Prompt2"]
...
@app.post("/api/update_agent_progress")
async def update_agent_progress(agent_name: str, progress: float):
    # Update the agent's progress in your backend state
    # This is a simplified example; you'll need to implement the actual state management
    for agent in agents:
